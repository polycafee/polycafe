...
// MasterChef is the master of Java Token (JAVA). He can make JAVA and he is a fair guy.
//
...
// contract once $JAVA is sufficiently distributed and the community can show to govern itself.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 0.02 JAVA per block.
    uint256 public constant INITIAL_EMISSION_RATE = 0.02 ether;
    // Max JAVA Supply
    uint256 public constant MAX_SUPPLY_CAP =  100000 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed __JavaPerBlock);
    ...
    constructor(
        ...
        JavaPerBlock = INITIAL_EMISSION_RATE;
    ...
    function getMultiplier(uint256 _from, uint256 _to)
        ...
        returns (uint256)
    {
        if (IERC20(java).totalSupply() + totalLockedUpRewards  >= MAX_SUPPLY_CAP) {
            return 0;
        }
    ...
    function pendingJava(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 JavaReward =
                multiplier.mul(JavaPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accJavaPerShare = accJavaPerShare.add(
                JavaReward.mul(1e12).div(lpSupply)
            );
        }
        uint256 pending =  user.amount.mul(accJavaPerShare).div(1e12).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest Java's.
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accJavaPerShare = pool.accJavaPerShare.add(
            JavaReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    // Deposit LP tokens to MasterChef for java allocation with referral.
    ...
    // Pay or lockup pending java.
    function payOrLockupPendingJava(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeJavaTransfer(msg.sender, totalRewards);
    ...
    function updateEmissionRate(uint256 _JavaPerBlock) public onlyOwner {
        ...
        emit UpdateEmissionRate(msg.sender, _JavaPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            java.mint(referrer, refBonusEarned);
